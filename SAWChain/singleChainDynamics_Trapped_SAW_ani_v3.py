...
if rep <= 2:
    ...
    anim.controls()
    if initConfig == "F": # Fully Extendedからスタートする場合
        savefile = "./gif/SingleChain_Dynamics_SAW_N{0}_FE_in-Tube_R{1}_T{2}steps".format(N, radi, rep)
    if initConfig == "R": # Random Coilからスタートする場合
        savefile = "./gif/SingleChain_Dynamics_SAW_N{0}_RC_in-Tube_R{1}_T{2}steps".format(N, radi, rep)
